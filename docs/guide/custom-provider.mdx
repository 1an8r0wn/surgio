...
|                     `trojan`                     | Trojan 订阅                                | 使用 V2rayN 订阅解析器，但只保留 Trojan 节点                                                                                |
|             `shadowsocks_subscribe`              | 通用的 Shadowsocks 订阅地址                |                                                                                                                              |
|             `shadowsocksr_subscribe`             | 通用的 Shadowsocksr 订阅地址               |                                                                                                                              |
|                `v2rayn_subscribe`                | V2rayN 订阅地址                            | 支持 VMess、Shadowsocks、SOCKS、VLESS、Trojan、Hysteria 2、TUIC、WireGuard、AnyTLS，以及内部格式的 HTTP/HTTPS                |
...
:::tip[提示]
如果你想了解如何编写更复杂的 Provider 请看 [这里](/guide/advance/advanced-provider)。
:::
...
Surfboard 支持 v1～v4，省略版本时采用客户端默认的 v1。v5 会告警并输出为 v4，与 Surfboard 官方说明一致。可设置 `obfsUri`，仅在 HTTP 混淆时输出；`udpRelay` 仅在 v3/v4 时生效。
...
```json5
{
  ...
  userkey: 'user-key', // 可选，多用户服务端的用户 key，仅 sing-box 使用
  mode: 'default', // 可选，v6 的流量整形模式：default、unshaped 或 unsafe-raw，仅 sing-box 使用
}
```

sing-box（1.14.0 及以上）需要显式设置 `version`，支持 v4 和 v6。v5 会告警并输出为 v4，因为 sing-box 不实现 v5 的 QUIC 模式，其余部分与 v4 协议相同。以下节点会告警并被忽略：未设置版本或版本为 v1～v3；v4 使用 `tls` 混淆（sing-box 只支持 `http` 混淆）；v6 的 `psk` 不在 12 到 255 字节之间。v6 不支持混淆，设置的 `obfs` 会被忽略；`mode` 只对 v6 生效。

Egern 支持 v1～v5（v5 按 v4 实现），v6 节点会告警并被忽略。
...
Surge、Loon 和 sing-box 支持输出多个 Peer；Stash 和 Clash 只会使用第一个 Peer。sing-box 会将 WireGuard 输出到顶层 `endpoints` 字段；模板需要使用 `getSingboxEndpoints`。Egern 只支持单个 Peer，包含多个 Peer 的节点会告警并被忽略。
  ...
  peers: [
    {
      ...
      allowedIps: '172.16.0.0/24', // 可选；sing-box 默认根据本地地址使用全路由
      keepalive: 25, // 可选
...
sing-box 将 Tailscale 视为 [endpoint](https://sing-box.sagernet.org/configuration/endpoint/tailscale) 而非 outbound，需要使用 `getSingboxEndpoints` 生成并放入配置的 `endpoints` 字段，详见 [sing-box 客户端文档](/guide/client/sing-box#tailscale-等-endpoint-节点)。

### Tuic

Surfboard 仅支持 v5，可设置 `udpRelay`、端口跳跃和 TLS 参数。`alpn` 缺省时由客户端采用 `h3`，目前仅接受一个值；多值 ALPN 的 Surfboard 语法尚未确认，因此会省略对应节点。
...
```json5
{
  ...
  congestionControl: 'bbr', // 可选，Clash 和 sing-box 支持
  udpRelay: false, // 可选，Surfboard v5 支持显式关闭，Surge 默认开启
}
```
...
```json5
{
  ...
  skipCertVerify: true, // 可选
  alpn: ['h3'], // 可选，Stash 不支持空值
  congestionControl: 'bbr', // 可选，Clash 支持
...
Surgio 只支持 Hysteria v2 协议。请注意，Hysteria v2 协议和 v1 协议完全不兼容。当前可以为 Clash、Surge、sing-box、Loon、Surfboard 和 Egern 生成此节点。
...
Surfboard 支持 `downloadBandwidth`、`portHopping`、`portHoppingInterval`、TLS 参数和 `udpRelay`，不输出 `uploadBandwidth`。Salamander 使用 `obfs: 'salamander'` 和 `obfsPassword`；Gecko 使用 `surfboardConfig.geckoPassword`，同时设置时优先 Gecko。
...
```json5
{
  ...
  obfs: 'salamander', // 可选
  obfsPassword: 'obfs-password', // 启用 obfs 时必填，Loon 输出为 salamander-password
  sni: 'sni.example.com', // 可选
  skipCertVerify: true, // 可选
...
Surfboard 支持密码、TLS 参数、`reuse` 和 `udpRelay`，后两项缺省时由客户端默认开启。配置 Reality 的节点不会输出到 Surfboard。

> <Badge text="Surgio v3.13.0" vertical="middle" />

当前支持为 Clash、Surge、sing-box、Quantumult X、Loon 和 Egern 生成 AnyTLS 节点。Loon 需要 Build 945 或更高版本。
...
```json5
{
  ...
  alpn: ['h2', 'http/1.1'], // 可选
  skipCertVerify: false, // 可选
  idleSessionCheckInterval: 0, // 可选，单位为秒
  idleSessionTimeout: 0, // 可选，单位为秒
...
- 订阅可以是明文或整段 Base64，节点可以使用协议标准 URI 或 `v2rayn://` 内部节点格式；
- 支持读取 VMess、Shadowsocks、SOCKS、VLESS、Trojan、Hysteria 2、TUIC、WireGuard 和 AnyTLS。内部格式的 HTTP/HTTPS 节点也可以读取；
- v2rayN 内部格式中的策略组、链式代理和 Outbound 不是独立代理节点，因此会被省略；
- 内部 WireGuard 节点没有提供 `WgInterfaceAddress` 时，使用与 v2rayN 相同的 `172.16.0.2/32` 默认地址；
- 若协议使用 Surgio 节点模型尚未表达的传输或混淆参数，该节点会被省略并记录警告，不会降级成错误配置；
:::
...
`trojan` Provider 与 `v2rayn_subscribe` 共用同一套订阅解析器，但只返回 Trojan 节点。它支持明文或整段 Base64 订阅，以及标准 URI 和 `v2rayn://` 内部节点格式；目前可映射 TCP 和 WebSocket 传输，其他传输会被省略并记录警告。
...
可用于 Loon 的 Shadowsocks 节点，包括 SS2022。生成 Loon 配置时会输出 `shadow-tls-password` 和 `shadow-tls-sni`；设置 `version` 时还会输出 `shadow-tls-version`，可指定为 `2` 或 `3`。

其他客户端的支持情况取决于协议和客户端实现。
...
开启 Tuic、Hysteria、Surge MASQUE 和 Stash TrustTunnel QUIC 协议端口跳跃，目前 Surge、sing-box、Stash、Mihomo、Surfboard 和 Egern 支持这一特性，Surfboard 和 Egern 适用于 TUIC v5 和 Hysteria2。例如 `5000,6000-7000`。该配置支持逗号或分号分割的端口列表，以及连字符分割的端口范围，Surgio 会自动转换成 Surge 和 Stash 支持的格式。Sing-box 的 Hysteria 协议也支持端口跳跃，但仅支持 `6000-7000` 这样连字符分割的端口范围，单个端口的配置会被忽略。MASQUE 端口跳跃仅输出到 Surge，且不能与 `underlyingProxy` 同时使用。TrustTunnel 端口跳跃仅输出到 Stash，并要求 `quic: true`。
...
:::tip[提示]
关于过滤器的自定义和其它进阶使用方法，请阅读 [「自定义过滤器」](/guide/advance/custom-filter)。
:::
...
[内置 `netflixFilter` 的解释](/guide/custom-template#netflixfilter)。
...
[内置 `youtubePremiumFilter` 的解释](/guide/custom-template#youtubepremiumfilter)。
...
:::tip[提示]
你现在可以定义 [全局的过滤器](/guide/custom-config#customfilters) 了！
:::
...
<a id="hooks-afternodelistresponse"></a>
