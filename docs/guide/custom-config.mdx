...
从 v2.7.0 开始，你可以在这里配置符合 [Surgio 片段](/guide/custom-template#片段-snippet) 格式的文件。我们以 Surgio 推荐的 [苹果服务规则](https://github.com/geekdada/surge-list/blob/master/surgio-snippet/apple.tpl) 为例。
...
<a id="clashconfig-enableshadowtls"></a>
...
默认开启 Vmess AEAD 加密，如果您的服务器不支持 AEAD 加密，请关闭。直接调用 `getSurfboardNodes` 时也使用此默认值。

### surfboardConfig.geckoPassword

- 类型：`string`，非空
- 默认值：未设置

仅用于 Surfboard 的 Hysteria2 Gecko 混淆，要求 Surfboard mobile 2.34.4+。可以在 Project 顶层设置，也可以在节点的 `surfboardConfig` 中覆盖。同一节点同时配置 Salamander 时，Gecko 优先。其他客户端输出不受此选项影响。

```ts
import { defineSurgioProject } from 'surgio/project'

export default defineSurgioProject({
  surfboardConfig: { geckoPassword: 'shared-gecko-password' },
  providers: {},
  artifacts: [],
})
```
...
3. 兜底副本与 Artifact 渲染缓存分开存放，缓存时长由 [`SURGIO_GATEWAY_ERROR_CACHE_MAXAGE`](/guide/env#surgio_gateway_error_cache_maxage) 控制，默认 1 天。
:::
...
:::warning[注意]
开启后 Artifact 的渲染结果不再缓存。客户端 `User-Agent` 的取值没有上限，按 `User-Agent` 缓存会让缓存条目数随访问量无限增长。
:::
...
白名单之外的请求头不会离开 Gateway，也不会参与 Artifact 渲染缓存的 key。请只加入真正会改变上游返回结果的 Header：每多一个取值不固定的 Header（例如 `Cookie`、`X-Forwarded-For`），缓存条目数就会翻一倍。
...
:::tip[提示]
1. 全局模板变量的用法和 Artifact 中定义的模板变量相同，相关文档请查阅 [这里](/guide/custom-artifact#customparams)；
...
定义缓存的实现方式。默认情况下，Surgio 将缓存持久化到系统临时目录中的文件；常驻的 Node 服务可以使用 Redis，Serverless 环境可以使用 Upstash REST，Cloudflare Worker 可以注入 KV binding。所有实现共享同一套 TTL 行为，TTL 的单位为毫秒。
...
- `redis`：通过 TCP 连接 Redis，仅适用于 Node
...
将 `cache.type` 设置为 `redis` 后，Surgio 优先读取配置中的 `redisUrl`，未配置时读取 `REDIS_URL` 环境变量。两者都缺失时，Surgio 在第一次访问缓存前报错。`redisUrl` 只接受 `redis://` 或 `rediss://`（TLS）开头的地址。
...
module.exports = {
  ...
  cache: {
    type: 'redis',
    redisUrl: process.env.MY_REDIS_URL,
  },
};
```

Redis 只能在 Node 中使用，Cloudflare Worker 无法建立 Redis TCP 连接。更多说明参见 [Redis 缓存](/guide/advance/redis-cache)。
...
Surgio 支持使用 [环境变量](/guide/env) 来调整没有公开的配置，但属于高级用法，请酌情使用。
