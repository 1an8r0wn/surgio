...
:::tip[提示]
请先参考 [文档](/guide/advance/api-gateway) 搭建托管 API
:::
...
:::tip[提示]
完整的 gateway 配置可以在 [这里](/guide/custom-config#gateway) 查看。
:::
...
直接打开 Gateway 的部署地址就可以看到面板。如果开启了鉴权则需登录。
...
| 参数       | 可选值                                           | 备注 |
| -------- | --------------------------------------------- | -- |
| `format` | `surge-policy`, `qx-server`, `clash-provider`, `egern` |    |
...
- `egern` Egern 代理列表（`proxies:`），可以在 Egern 的 `external` 策略组中引用
